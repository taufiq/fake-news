...
def pipeline_test(test, bow_vectorizer, tfreq_vectorizer, tfidf_vectorizer):
    ...
    for instance in test.instances:
        ...
        if head not in heads_track:
            ...
            head_tf = tfreq_vectorizer.transform(head_bow).toarray()[
                0].reshape(1, -1)
            head_tfidf = tfidf_vectorizer.transform(
        ...
        if body_id not in bodies_track:
            body_bow = bow_vectorizer.transform(
                [test.bodies[body_id]]).toarray()
...
def save_predictions(pred, file):
    ...
    with open(file, 'w') as csvfile:
        ...
        for instance in pred:
            verdict = label_ref_rev[instance]
            csvfile.write(verdict)
    return verdict
